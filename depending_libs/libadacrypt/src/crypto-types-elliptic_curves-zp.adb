...
package body  Crypto.Types.Elliptic_Curves.Zp is
   ...
   function "+"(Left, Right : EC_Point) return EC_Point is
	begin
      ...
      declare
         Result : EC_Point;
         Temp : Big_Unsigned := Div(Sub(Right.Y,Left.Y,P),Sub(Right.X,Left.X,P),P);
      begin
   ...
   begin
      if Is_Negative(X) then --X = Negative(X) then
         ...
         declare
            Result : EC_Point;
            Temp : Big_Unsigned := Div(Add(Mult(Mult(X.X,X.X,P),Big_Unsigned_Three,P),A,P),Mult(X.Y,Big_Unsigned_two,P),P);
				--s : Big_Unsigned :=  
         begin
   ...
   function "*"(Left : Big_Unsigned; Right : EC_Point) return EC_Point is
      W					: EC_Point;
      M 					: EC_Point  := Right;
      K 					: Mod_Types :=  Big.Utils.To_Mod_Types(Left);
-------------------------------------------------------------------------
--      Q				: EC_Point;
--      P 				: EC_Point  := Right;
