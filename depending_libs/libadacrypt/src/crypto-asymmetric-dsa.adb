...
use  Crypto.Hashfunction_SHA1, Ada.Text_Io;
...
--  package WIO is new Ada.Text_IO.Modular_IO (Word);
   ...
   procedure Sign(Private_Key : in  Private_Key_DSA;
      ...
      K : Big_Unsigned;
      H : constant Big_Unsigned
        := To_Big_Unsigned(To_Bytes(Sha1_Hash));
   ...
   function Verify(Public_Key  : Public_Key_DSA;
      ...
      W  : Big_Unsigned := Inverse(Signature.S, Public_Key.Q);

      U1 : Big_Unsigned :=
        Mult(To_Big_Unsigned(To_Bytes(Sha1_Hash)), W,Public_Key.Q);

      U2 : Big_Unsigned := Mult(Signature.R, W, Public_Key.Q);

      V : Big_Unsigned:= Mult(Pow(Public_Key.G, U1, Public_Key.P),
 ...
 procedure Get_Private_Key(Private_Key : in Private_Key_DSA;
   ...
   begin
      if Is_Init(DSA_Key(Private_Key)) then
         Sign(Private_Key, F_Hash(Filename), Signature);
      else
         raise Invalid_Private_Key_Error;
      end if;
   end Sign_File;
