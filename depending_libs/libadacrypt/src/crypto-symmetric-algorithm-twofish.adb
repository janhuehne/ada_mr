...
--   package BIO is new Ada.Text_Io.Modular_IO (Byte);
   ...
   procedure Prepare_Key(Key : in Bytes;
      ...
      Kcycles : Key_Cycles_Type := Shift_Right(Byte(Key'Last+7),3);
      I : Byte:=0;
      A, B : Word;
      C, Bx, Bxx  :  Byte;
 ...
 --
   ...
   begin
      ...
      Ciphertext( 0.. 3) := R_To_Bytes(C);
      Ciphertext( 4.. 7) := R_To_Bytes(D);
      Ciphertext( 8..11) := R_To_Bytes(A);
      Ciphertext(12..15) := R_To_Bytes(B);
   ...
   begin
      ...
      Plaintext( 0.. 3) := R_To_Bytes(C);
      Plaintext( 4.. 7) := R_To_Bytes(D);
      Plaintext( 8..11) := R_To_Bytes(A);
      Plaintext(12..15) := R_To_Bytes(B);
   ...
   begin
      Prepare_Key(Key,Cipherkey.Roundkey);
   end Prepare_Key128;
