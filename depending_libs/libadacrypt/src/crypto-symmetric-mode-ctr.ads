...
generic
   with package C is new Crypto.Symmetric.Blockcipher(<>);

   with function "+"   (Left : C.Block; Right : Byte) return C.Block is <>;
   with function "xor" (Left, Right  : in C.Block)    return C.Block is <>;

package Crypto.Symmetric.Mode.CTR is
   use C;
