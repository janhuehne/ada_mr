...
--============================================================================--
   function "+"(Left, Right : Big_Unsigned) return Big_Unsigned is
      Result : Big_Unsigned;
      L      : Natural := Natural'Max( Bit_Length(Left) , Bit_Length(Right) );
   begin
...
--============================================================================--
   ...
   function "*"(Left, Right : Big_Unsigned) return Big_Unsigned is
      Result : Big_Unsigned ;
      L : Natural := Bit_Length(Left)+Bit_Length(Right);
   begin
   ...
   begin
       if Bit_Length(Left)+Bit_Length(Right) < Mod_Type'Size then
         ...
         declare
            ...
            N                : Natural := Natural'Max( Bit_Length(Left)
                                                     , Bit_Length(Right))/2;
         begin
...
--            P_1     := Karatsuba(Left_1, Right_1);
--            P_2     := Karatsuba(Left_2, Right_2);
--            Result  :=  Shift_Left(P_1, 2*N)
--                     +  Shift_Left((Karatsuba((Left_1  + Left_2) ,
--                                   (Right_1 + Right_2)) - P_1 - P_2), N)
--                     +   P_2;
   ...
   begin
       if Bit_Length(Left)+Bit_Length(Right) < Mod_Type'Size then
         ...
         declare
            ...
            N                : Natural := Natural'Max( Bit_Length(Left) , Bit_Length(Right))/2;
            -----------------------------------------------------------------------
...
--   function Karatsuba_Prot (Left, Right : Big_Unsigned) return Big_Unsigned is
--      Result           : Big_Unsigned;
--   begin
--      if Bit_Length(Left)+Bit_Length(Right) < Mod_Type'Size then
--         Result.Number(0) := Left.Number(0) * Right.Number(0);
...
--            N                : Natural := Natural'Max( Bit_Length(Left) , Bit_Length(Right))/2;
--            -----------------------------------------------------------------------
--            protected type Kara_Prot_Type is
--               function Arbeite (Left, Right : Big_Unsigned) return Big_Unsigned;
--            end Kara_Prot_Type;
--            protected body Kara_Prot_Type is
--               function Arbeite (Left, Right : Big_Unsigned) return Big_Unsigned is
--               begin
--                  Result := Karatsuba_Prot(Left, Right);
--                  return Result;
--               end Arbeite;
--            end Kara_Prot_Type;
--            Task_1 : Kara_Prot_Type;
--            Task_2 : Kara_Prot_Type;
--            Task_3 : Kara_Prot_Type;
--            -----------------------------------------------------------------------
--         begin
--            Left_1  := Shift_Right(Left, N);
--            Left_2  := Left - Shift_Left( Left_1, N );
...
--            Right_2 := Right - Shift_Left( Right_1, N );
--
--            P_1 := Task_1.Arbeite(Left_1, Right_1);
--            P_2 := Task_2.Arbeite(Left_2, Right_2);
--            P_3 := Task_3.Arbeite((Left_1  + Left_2), (Right_1 + Right_2));
--
--            Result :=  Shift_Left(P_1, 2*N)
--                    +  Shift_Left((P_3 - P_1 - P_2), N)
--                    +   P_2;
...
--------------------------------------------------------------------------------
   ...
   begin
      ...
      declare
         ...
         Length                : Natural  := Natural'Max( Bit_Length(Left) ,
                                                          Bit_Length(Right) );
         N              : Natural  := Length / 3;
         DN             : Natural  := 2 * N;
   ...
   -- EVALUATION ...............................................................
--       F_Left_I  := L_0*(X**2) + L_1*(X) + L_2;
--       F_Right_I := R_0*(X**2) + R_1*(X) + R_2;
--       with X := 0, 1, 2, 3, 4;
...
--         knuth_1(1) := Toom_Cook(     L(2), R(2)      );
--         knuth_1(2) := Toom_Cook(L(0) + L(1) + L(2), R(0) + R(1) + R(2));
--         knuth_1(3) := Toom_Cook(F_Left(2), F_Right(2));
--         knuth_1(4) := Toom_Cook(F_Left(3), F_Right(3));
--         knuth_1(5) := Toom_Cook(F_Left(4), F_Right(4));
--         knuth_1(1) := Karatsuba(     L(2), R(2)      );
--         knuth_1(2) := Karatsuba(L(0) + L(1) + L(2), R(0) + R(1) + R(2));
--         knuth_1(3) := Karatsuba(F_Left(2), F_Right(2));
--         knuth_1(4) := Karatsuba(F_Left(3), F_Right(3));
--         knuth_1(5) := Karatsuba(F_Left(4), F_Right(4));
--         knuth_1(1) := Karatsuba_P(     L(2), R(2)      );
--         knuth_1(2) := Karatsuba_P(L(0) + L(1) + L(2), R(0) + R(1) + R(2));
--         knuth_1(3) := Karatsuba_P(F_Left(2), F_Right(2));
--         knuth_1(4) := Karatsuba_P(F_Left(3), F_Right(3));
--         knuth_1(5) := Karatsuba_P(F_Left(4), F_Right(4));
         knuth_1(1) :=       L(2)* R(2);
...
--               X := Toom_Cook(Left_Local, Right_Local);
--               X := Karatsuba(Left_Local, Right_Local);
--               X := Karatsuba_P(Left_Local, Right_Local);
               X := Left_Local * Right_Local;
...
--============================================================================--
   ...
   ---------------------------------------------------------------------------
    ...
    function "xor"(Left, Right : Big_Unsigned) return Big_Unsigned is
       Result : Big_Unsigned;
       M : Natural:= Natural'Max(Left.Last_Index, Right.Last_Index);
    begin
       -- xor
       for I in  0..M loop
   ...
   ---------------------------------------------------------------------------
    ...
    function "and"(Left, Right : Big_Unsigned) return Big_Unsigned is
       Result : Big_Unsigned;
       M : Natural:= Natural'Min(Left.Last_Index, Right.Last_Index);
    begin
