...
package body Crypto.Symmetric.Algorithm.Blowfish is
   ...
   begin
      ...
      Plaintext (0 .. 3) := To_Bytes (Pieces (0));
      Plaintext (4 .. 7) := To_Bytes (Pieces (1));
   end Decrypt128;
   ...
   begin
      ...
      Ciphertext (0 .. 3) := To_Bytes (Pieces (0));
      Ciphertext (4 .. 7) := To_Bytes (Pieces (1));
   end Encrypt128;
   ...
   begin
      ...
      for I in Cipherkey.P'Range loop
         if Part_Index + 3 > Key'Last then
            ...
            Part (0 .. Last) := Key (Part_Index .. Key'Last);
            Part (Last + 1 .. 3) :=
              Key (Key'First .. Key'First + 2 - Last);
            Part_Index := Key'First + 3 - Last;
         else
            Part := Key (Part_Index .. Part_Index + 3);
   ...
   is
   begin
      Pieces (0) := To_Word (Block (0 .. 3));
      Pieces (1) := To_Word (Block (4 .. 7));
   end To_Words;
