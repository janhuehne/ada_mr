...
package body Mod_Utils is
   ...
   function Patch(Item, N : Big_Unsigned)
      ...
      Diff : Big_Unsigned:=((Big_Unsigned_Last - N) + 1) mod N;
   begin
      ...
      for I in 0..N.Last_Index loop
         for J in 0..(Shift_Right(Mod_Type(Mod_Type'Size),K)-1) loop
            ...
            Result.Number(I) := Result.Number(I) or
              Shift_Left(Mod_Type(B),Natural(Shift_Left(Mod_Type(J),K)));
         end loop;
      end loop;
   ...
   begin
      ...
      declare
         P : Mod_Type := Mod_Type(Bit_Length(X)-1);
      ...
      begin
         if Status = Weak then
            for I in 1..Rounds loop
               ...
               if Is_Simple_Witness(Witness,X) = True then return False;
               end if;
            end loop;
         else
            for I in 1..Rounds loop
               ...
               if Is_Miller_Rabin_Witness(Witness,X) = True then return False;
               end if;
            end loop;
         end if;
      end;
  ...
  ----------------------------------------------------------------------------
   ...
   begin
      ...
      declare
         ...
         L : Natural := Amount mod Mod_Type'Size;
         R : Natural := Mod_Type'Size-L;
         M : Natural := Amount/Mod_Type'Size;
      begin
