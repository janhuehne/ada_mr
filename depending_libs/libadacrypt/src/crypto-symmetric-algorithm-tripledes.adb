...
--   package B48 is new Ada.Text_Io.Modular_IO (Bit48);
   ...
   type Bit48 is mod 2**48;
   for Bit48'Size use 48;

   type Bit7 is mod 128;
   for Bit7'Size use 7;
   ...
   --Bit7-blocks (S_BlockN): array of N/7 Bit7s
   subtype B7_Block28 is Bit7s (0 .. 3);
   ...
   begin
      Obsolete_Build_Encrypt_Key(Key( 0.. 7), Cipherkey(0).Encrypt_Key);
      Obsolete_Build_Encrypt_Key(Key( 8..15), Cipherkey(1).Encrypt_Key);
      Obsolete_Build_Encrypt_Key(Key(16..23), Cipherkey(2).Encrypt_Key);
   ...
   procedure Build_Encrypt_Key(Key : in B_Block64; Kn : out Roundkey_DES) is
      Direction : Direction_Type := Encrypt;
   begin
   ...
   procedure Build_Decrypt_Key(Key : in B_Block64; Kn : out Roundkey_DES) is
      Direction : Direction_Type := Decrypt;
   begin
      Build_Roundkey(Key, Kn, Direction);
   ...
   begin
      ...
      Build_Encrypt_Key(Key( 0.. 7), Cipherkey(0).Encrypt_Key);
      Build_Encrypt_Key(Key( 8..15), Cipherkey(1).Encrypt_Key);
      Build_Encrypt_Key(Key(16..23), Cipherkey(2).Encrypt_Key);


      Build_Decrypt_Key(Key( 0.. 7), Cipherkey(0).Decrypt_Key);
      Build_Decrypt_Key(Key( 8..15), Cipherkey(1).Decrypt_Key);
      Build_Decrypt_Key(Key(16..23), Cipherkey(2).Decrypt_Key);
