...
package body Binfield_Utils is
   ...
   function B_Add(Left,Right : Big_Unsigned) return Big_Unsigned is
      N : Natural := Natural'Max(Left.Last_Index, Right.Last_Index);
      C : Big_Unsigned;
   begin
      for I  in 0..N loop
   ...
   function B_Square(A, F : Big_Unsigned) return Big_Unsigned is
      K : Natural := Shift_Right(Mod_Type'Size,3);
      N : Natural := Shift_Right(K,1)-1;
      M : Natural := Bit_Length(F);
      L : Natural;
      C : D_Big_Unsigned;
   begin
      if K = 1 then
