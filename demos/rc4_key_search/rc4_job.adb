  ...
  begin
    ...
    for I in 0 .. (255 / Most_Sig_Byte_Range) loop
      ...
      declare
        The_Job : Job;
      begin
        The_Job.Plain_Text          := Plain_Text;
        The_Job.Cipher_Text         := Cipher_Text;
        The_Job.Start_Most_Sig_Byte := Rc_4.Unsigned_Byte(I * Most_Sig_Byte_Range);
        
        if Integer(The_Job.Start_Most_Sig_Byte) + Most_Sig_Byte_Range - 1 > 255 then
          The_Job.Most_Sig_Byte_Range := 255 - Integer(The_Job.Start_Most_Sig_Byte);
        else
          The_Job.Most_Sig_Byte_Range := Most_Sig_Byte_Range - 1;
        end if;
