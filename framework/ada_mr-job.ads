...
package Ada_Mr.Job is
  ...
  -- record
  type Job is abstract tagged record
    Job_Id : Natural;
  end record;
  
  -- Serializes a job into the xml format
  function To_Xml(The_Job : Job) return String is abstract;
  
  -- Deserializes a job from the xml format
  function From_Xml(Xml_Node : Ada_Mr.Xml.Node_Access) return Job is abstract;
  
  --  Returns the id from a given job
  function Get_Job_Id(The_Job : Job) return Natural;
  ...
  function Get_Next_Raw_Job return Job is abstract;
  
  -- Called by the generic system to print job in stio
  procedure Print_Job(The_Job : Job; State : String) is abstract;
  
  -- Called by the generic mapper to compute a job
  procedure Compute_Job(The_Job : Job) is abstract;
